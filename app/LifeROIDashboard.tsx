...
type UploadSummary = { id:number; source:string; money:number; screenMinutes:number; energyKwh:number; energyCost:number; subscriptions:number; records:number; confidence:number };
const freshSummary = (source="Synthetic cross-resource statement"): UploadSummary => ({
  id:Date.now(), source, money:72000+Math.round(Math.random()*24000), screenMinutes:250+Math.round(Math.random()*150),
  energyKwh:330+Math.round(Math.random()*190), energyCost:3000+Math.round(Math.random()*1700),
  subscriptions:2200+Math.round(Math.random()*2600), records:34+Math.round(Math.random()*28), confidence:90+Math.round(Math.random()*8),
});
...
export default function LifeROIDashboard() {
  ...
  return <main className="app-shell">
    ...
    <section className="content" id="overview">
      ...
      <section className="time-details tab-section" hidden={!dataAvailable.time || active!=="Time"} aria-label="Time spent by activity"><article className="panel time-allocation"><div className="card-top"><div><p className="eyebrow">YOUR 24 HOURS 🕰️</p><h3>Where your day goes</h3></div><button className="inline-info" aria-label="About time data" onClick={()=>setInfo("time")}>i</button></div><div className="time-summary"><div className="time-donut"><span><b>24h</b><small>ONE DAY</small></span></div><div className="time-totals"><span><b>8h</b><small>work</small></span><span><b>7h 30m</b><small>sleep</small></span><span><b>5h 30m</b><small>screen time</small></span><span><b>3h</b><small>family & other</small></span></div></div></article><article className="panel activity-list"><div className="card-top"><div><p className="eyebrow">ACTIVITY BREAKDOWN</p><h3>Daily time spent</h3></div><span className="positive">7-day average</span></div>{timeActivities.map(activity=><button key={activity.name} onClick={()=>setNotice(`${activity.icon} ${activity.name}: ${Math.floor(activity.minutes/60)}h ${activity.minutes%60 ? `${activity.minutes%60}m` : ""} per day · ${Math.round(activity.minutes*365/60)} hours/year`)}><span className="activity-icon" style={{background:`${activity.color}22`}}>{activity.icon}</span><div><label><b>{activity.name}</b><strong>{Math.floor(activity.minutes/60)}h {activity.minutes%60 ? `${activity.minutes%60}m` : ""}</strong></label><i><span style={{width:`${Math.min(100,activity.minutes/480*100)}%`,background:activity.color}} /></i><small>{Math.round(activity.minutes*7/60)} hours/week · {Math.round(activity.minutes*365/60)} hours/year</small></div></button>)}</article><article className="panel weekly-rhythm"><div className="card-top"><div><p className="eyebrow">WEEKLY RHYTHM 📅</p><h3>Screen time by day</h3></div><b>avg 5h 30m</b></div><div className="week-bars">{[["M",70],["T",62],["W",78],["T",55],["F",84],["S",100],["S",91]].map(([day,value],index)=><button key={`${day}-${index}`} onClick={()=>setNotice(`${["Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"][index]}: ${Math.round(Number(value)*3.8)} minutes screen time`)}><i style={{height:`${value}%`}}/><span>{day}</span></button>)}</div><div className="rhythm-insight"><span>🌤️</span><p><b>Saturday is your highest-screen-time day.</b> Reclaiming 45 minutes on weekends alone creates nearly 78 hours a year.</p></div></article></section>
