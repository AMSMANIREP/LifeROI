...
const nav = [["⌂","Overview"],["₹","Money"],["◷","Time"],["ϟ","Energy"],["♻","Resources"],["◇","Future"],["◎","Goals"],["✦","Opportunities"],["🧭","ROI Compass"],["⚙","Settings"]];
...
const viewCopy: Record<string, { title:string; subtitle:string; emoji:string }> = {
  ...
  "ROI Compass":{title:"LifeROI Compass",subtitle:"A curious guide that connects your money, time, energy, and future.",emoji:"🧭"},
...
export default function LifeROIDashboard() {
  ...
  function navigate(label: string) {
    if (label === "ROI Compass") { setAdvisor(true); window.setTimeout(() => advisorInputRef.current?.focus(), 80); return; }
    setActive(label);
  ...
  return <main className="app-shell">
    ...
    <section className="content" id="overview">
      ...
      <section className="upload-workspace panel tab-section" hidden={active!=="Upload"} id="upload"><div className="upload-intro"><span className="upload-planet">📥</span><p className="eyebrow">RESOURCE DATA INBOX</p><h2>Turn a document into understanding.</h2><p>Your source stays private. LifeROI extracts a draft first, then waits for your review before adding anything to the dashboard.</p><div className="upload-types"><span>📷 Screenshot</span><span>📄 PDF</span><span>📊 CSV</span><span>📝 Text</span></div></div><div className="dropzone upload-dropzone"><span>↥</span><h3>Drop your resource data here ✨</h3><p>Bank statement, screen-time image, utility bill, or subscription summary · up to 10 MB</p><button className="primary" onClick={()=>{setUploadReady(true);celebrate("✨ Demo statement understood — your review is ready!",["📥","✨","🔍","✅"])}}>Analyze a demo statement</button></div><div className={`ingestion-journey ${uploadReady?"complete":""}`}>{[["📖","Reading document"],["🧠","Understanding type"],["🔎","Extracting data"],["🏷️","Categorizing"],["✅","Ready to review"]].map(([icon,label],index)=><div key={label}><span>{icon}</span><i>{index+1}</i><b>{label}</b></div>)}</div>{uploadReady && <div className="review-ready"><span>🎉</span><div><b>Analysis ready</b><p>12 transactions found · Money statement · 94% average confidence</p></div><button onClick={()=>{navigate("Money");setNotice("✅ Demo data confirmed and added to Money")}}>Review & confirm →</button></div>}<div className="modal-note"><b>🔒 Private by design</b><span>Files are user-isolated, never written to ordinary logs, and can be deleted after extraction.</span></div></section>
    ...
    <button className="advisor-launcher compass-launcher" onClick={()=>navigate("ROI Compass")} aria-label="Open LifeROI Compass"><span>🧭</span><b>Open Compass</b><i>✦</i></button>
